...
import { userManagementStore, AdminFilterState } from "@/store/userManagementStore";

interface AdminFiltersProps {
  onApplyFilter: (filters: AdminFilterState) => void;
}

const AdminFilters: React.FC<AdminFiltersProps> = ({ onApplyFilter }) => {
  const { adminFilters, clearAdminFilters } = userManagementStore();
  ...
  const [fromDate, setFromDate] = useState<Date | undefined>(adminFilters.fromDate);
  const [toDate, setToDate] = useState<Date | undefined>(adminFilters.toDate);

  // Sync with store when filters change externally
  useEffect(() => {
    setSelectedStatus(adminFilters.status);
    setFromDate(adminFilters.fromDate);
    setToDate(adminFilters.toDate);
  }, [adminFilters]);
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative w-full">
              <button
                onClick={() => setAccountStatusOpen(!accountStatusOpen)}
                className="w-full p-2 bg-white border border-[#B5B1B1] rounded-[8px] flex items-center justify-between text-sm font-normal hover:border-gray-400 transition-colors"
                type="button"
              >
